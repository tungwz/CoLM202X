...
CONTAINS
   ...
   ! -------
   SUBROUTINE flux_map_and_write_tg ( &
         acc_vec, file_hist, varname, itime_in_file, sumarea, filter, &
         longname, units)

   USE MOD_Block
   USE MOD_Vars_1DAccFluxes,  only: nac_tg
   USE MOD_Vars_Global, only: spval
   IMPLICIT NONE
   ...
   integer :: compress
      ...
      IF (p_is_worker) THEN
         DO i = lbound(acc_vec,1), ubound(acc_vec,1)
            IF ((acc_vec(i) /= spval) .and. (nac_tg(i) > 0)) THEN
               acc_vec(i) = acc_vec(i) / nac_tg(i)
            ENDIF
         ENDDO
      ENDIF
   ...
   END SUBROUTINE flux_map_and_write_tg

   ! -------
   SUBROUTINE flux_map_and_write_urb_tduirl ( &
         acc_vec, file_hist, varname, itime_in_file, sumarea, filter, &
         longname, units)
   ...
   integer :: compress
      ...
      IF (p_is_worker) THEN
         DO i = lbound(acc_vec,1), ubound(acc_vec,1)
            IF ((acc_vec(i) /= spval) .and. (nac_24(i) > 0)) THEN
               acc_vec(i) = acc_vec(i) / nac_24(i)
            ENDIF
         ENDDO
      ENDIF
