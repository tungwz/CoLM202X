...
!-----------------------------------------------------------------------
         ...
         IF (numlay .eq. 3) THEN
            ...
            taf(2) = ((tg*fg/rd(1)+tu(3)*fc(3)*lsai/rb(3)+Hahe(1)/(rhoair*cpair))*aT &
                     ...
                     + Hahe(2)/(rhoair*cpair)) &
                     / (cT*(1-aT*fg/(cT*rd(2))-bT*fg/(cT*rd(3))))

            taf(1) = (taf(2)*fg/rd(2) + tg*fg/rd(1) + tu(3)*fc(3)*lsai/rb(3) &
            ...
            Lahe   = (Fhac + Fwst + vehc)*flh
            aQ     = 1/(rd(2)*(fg/rd(2)+fg*fgper/(rd(1)+rss_)+fwet_gimp*fg*fgimp/rd(1)+fc(3)/rv))
            bQ     = 1/(rd(3)*(1/raw+fg/rd(3)+fwet_roof*fc(0)/rb(0)))
            cQ     = 1/rd(3) + 1/rd(2)

            qaf(2) = ( (qgper*fg*fgper/(rd(1)+rss_) + qgimp*fwet_gimp*fg*fgimp/rd(1) +  &
                     + qsatl(3)*fc(3)/rv + Lahe/rhoair/hvap)*aQ &
                     + (qm/raw+qsatl(0)*fwet_roof*fc(0)/rb(0))*bQ ) &
                     / ( cQ*(1-aQ*fg/(cQ*rd(2))-bQ*fg/(cQ*rd(3))) )

            qaf(1) = ( qaf(2)*fg/rd(2) + qgper*fg*fgper/(rd(1)+rss_) &
                     + qgimp*fwet_gimp*fg*fgimp/rd(1) + qsatl(3)*fc(3)/rv + Lahe/rhoair/hvap ) &
                     / ( fg/rd(2) + fg*fgper/(rd(1)+rss_) + fwet_gimp*fg*fgimp/rd(1) + fc(3)/rv )
...
! sensible heat fluxes and their derivatives
         ...
         IF (botlay == 2) THEN
            fsenl_dtl = rhoair * cpair * lsai/rb(3) &
                      * ( 1. - fc(3)*lsai/(cT*rb(3)*(1-bT*fg/(cT*rd(3)))) )
         ELSE
            fsenl_dtl = rhoair * cpair * lsai/rb(3) &
...
! latent heat fluxes and their derivatives
         ...
         ELSE
            etr_dtl = rhoair * (1.-fwet) * delta * lai/(rb(3)+rs) &
                    * ( 1. - fc(3)*aQ*aQ/(rv*cQ*(1-aQ*fg/(cQ*rd(2))-bQ*fg/(cQ*rd(3)))) &
                           - fc(3)/(rv*(fg/rd(2) + fg*fgper/(rd(1)+rss_) + &
                                    fwet_gimp*fg*fgimp/rd(1) + fc(3)/rv)) )
         ENDIF
...
ENDIF
         ...
         ELSE
            evplwet_dtl = rhoair * (1.-delta*(1.-fwet)) * lsai/rb(i) &
                        * ( 1. - fc(3)*aQ*aQ/(rv*CQ*(1-aQ*fg/(cQ*rd(2))-bQ*fg/(cQ*rd(3)))) &
                               - fc(3)/(rv*(fg/rd(2) + fg*fgper/(rss_+rd(1)) + &
                                        fwet_gimp*fg*fgimp/rd(1) + fc(3)/rv)) )
         ENDIF
...
! canopy air humidity
         ...
         IF (numlay .eq. 3) THEN
            ...
            taf(2) = ((tu(3)*fc(3)*lsai/rb(3)+tg*fg/rd(1)+Hahe(1)/(rhoair*cpair))*aT &
                     + tu(1)*fc(1)/rb(1) + tu(2)*fc(2)/rb(2) &
            ...
            qaf(1) = (qaf(2)*fg/rd(2)+qgper*fg*fgper/(rd(1)+rss_)+qgimp*fwet_gimp*fg*fgimp/rd(1) &
                     + qsatl(3)*fc(3)/rv+Lahe/rhoair/hvap) &
                     /(fg/rd(2)+fg*fgper/(rd(1)+rss_)+fwet_gimp*fg*fgimp/rd(1)+fc(3)/rv)
